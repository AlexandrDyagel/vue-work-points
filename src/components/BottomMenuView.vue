...
const styleBgBottomMenu = computed(() => {
  if (route.path === Route.TaskMap) return 'bg-[#242528]/70'
  else return 'bg-[#242528]'
})

const styleItemsBottomMenu = ((isActive:boolean):string => {
  if (isActive) {
    if (route.path !== Route.TaskMap) return 'text-[#5fb336] scale-110 bg-[#242528]'
    else return 'text-[#5fb336] scale-110 bg-[#242528]/0'
  } else {
    if (route.path !== Route.TaskMap) return 'bg-[#242528]'
    else return 'bg-[#242528]/0'
  }
})
...
<template>
  ...
  <div class="fixed start-0 bottom-0 w-full z-20" :class="styleBgBottomMenu">
    <div
