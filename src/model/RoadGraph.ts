...
export class RoadGraph {
  ...
  findPath(startNodeId: string, endNodeId: string): [number, number][] | null {
    ...
    fScore.set(
      startNodeId,
      this.calculateDistance(startNode.lat, startNode.lng, endNode.lat, endNode.lng),
    )
    ...
    while (openSet.size > 0) {
      ...
      for (const neighborId of currentNode.connections) {
        ...
        const tentativeGScore =
          ...
          this.calculateDistance(currentNode.lat, currentNode.lng, neighbor.lat, neighbor.lng)
        ...
        if (tentativeGScore < (gScore.get(neighborId) ?? Infinity)) {
          ...
          fScore.set(
            neighborId,
            tentativeGScore +
              this.calculateDistance(neighbor.lat, neighbor.lng, endNode.lat, endNode.lng),
          )
